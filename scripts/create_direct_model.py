"""
Create a direct model without custom transformers for more reliable deployment.
...
# This helps the model learn realistic pricing without requiring a custom transformer
print("Creating expected price feature...")
df['expected_price'] = (
    500 +  # Base price
    df['RAM_GB'] * 15 +  # RAM cost
    df['SSD_GB'] * 0.15 +  # SSD cost
    df['Procesador_Número de núcleos del procesador'].fillna(4).astype(float) * 50 +  # CPU cost
...
for config in test_configs:
    ...
    test_df = pd.DataFrame({
        'RAM_GB': [config["ram"]],
        'Procesador_Número de núcleos del procesador': [config["cpu"]],
        'SSD_GB': [config["ssd"]],

"""
Price scaling module for the ML Marketplace.

This module provides a custom price scaler class that handles scaling and inverse scaling
of price data for prediction models.
"""
...
class PriceScaler(BaseEstimator, TransformerMixin):
    ...
    correctly inverse-transforming them during prediction.
    """
    
    def __init__(self, price_column='Price'):
        """
        Initialize the price scaler.
    ...
    def inverse_transform(self, X, y_pred):
        ...
        if self.price_mean_ is not None and self.price_std_ is not None:
            ...
            if hasattr(raw_price, '__iter__'):
                ...
                return np.maximum(np.abs(raw_price), 100)
            else:
                # For scalar values
                return max(abs(raw_price), 100)
        else:
            # Similar handling for y_pred
            if hasattr(y_pred, '__iter__'):
                return np.maximum(np.abs(y_pred), 100)
            else:
                return max(abs(y_pred), 100)
